...
function updateSpecificIngredient(updatedDatas: IUpdatedIngredient) {
  ...
  const indexIngredient = data.value.findIndex(
    (ingredient: any) => ingredient.id === updatedIngredient.id
  )
...
<template>
  <div>
    <div class="flex items-center py-4">
      <Input
        class="max-w-sm"
        :placeholder="informationsInputSearch.placeHolder"
        :model-value="
          table.getColumn(informationsInputSearch.searchProperty)?.getFilterValue() as string
        "
        @update:model-value="
          table.getColumn(informationsInputSearch.searchProperty)?.setFilterValue($event)
        "
      />
      <DropdownMenu>
    ...
    <FormIngredient
      v-if="showFormIngredient && selectedIngredient"
      :selectdIngredient="selectedIngredient"
      @close="closeFormIngredient()"
      @completed="updateSpecificIngredient"
    >
    </FormIngredient>
    <div class="border rounded-md">
      <Table
        :columns="columns"
        :informationsInputSearch="informationsInputSearch"
        :dataProps="dataProps"
      >
        <TableHeader>
