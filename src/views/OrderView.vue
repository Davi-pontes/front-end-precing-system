...
async function sendOrder() {
  try {
    ...
    const datasFormated = {
      orderItems: formatedItemsSelectedForSend,
...
function handleItemSelected(item: ICommandItem) {
  ...
  if (productAlreadyAdded) {
    ...
    itemsSelected.value[index].currentPrice =
      itemsSelected.value[index].price_per_unit * itemsSelected.value[index].quantity
    ...
    if (itemInAlluserProduct) {
      itemsSelected.value.push({
        ...itemInAlluserProduct,
        quantity: 1,
        currentPrice: itemInAlluserProduct.price_per_unit
      })
  ...
  <div class="flex flex-col w-full">
    <div class="flex w-full h-[32em] gap-2">
      ...
      <div class="w-[75%] h-full border shadow-lg rounded-md p-4">
        ...
        <div class="flex flex-col w-full h-[15em]" v-if="itemsSelected.length > 0">
          <ul
            class="flex w-full items-center justify-between gap-x-4 border-b pb-2 mt-1"
            v-for="item of itemsSelected"
            :key="item.id_product"
          >
        ...
        <div class="flex w-full gap-4">
          <div>
            ...
            <RadioGroup
              class="mt-2"
              :items="paymentMethodFormatedToComboBox"
    ...
    <div class="w-full border shadow-lg rounded-md p-4 mt-7">
      ...
      <TableComponent
        ...
        :dataProps="allUserORder"
        :informationsInputSearch="{
          placeHolder: 'Filtre pelo número do pedido.',
          searchProperty: 'id'
        }"
      >
      </TableComponent>
